...
def sym2upper(X : Tensor) -> Tensor:
    """Takes upper triangular elements along last 2 dimensions.
    Offdiagonal elements are multiplied to preserve the norm.
    """
    assert X.ndim >= 2
    ...
    x_vec[...,ixs[0] != ixs[1]] *= math.sqrt(2)
    return x_vec

def upper2sym(x_vec : Tensor) -> Tensor:
    """Takes upper triangular elements along last 2 dimensions.
    Offdiagonal elements are multiplied to preserve the norm.
    """

    ndim = (math.sqrt(1 + 8 * x_vec.shape[-1]) - 1) / 2
    ...
    # multiply offdiagonal elements to preserve the norm
    X[...,ixs[0,od_mask], ixs[1,od_mask]] /= math.sqrt(2)
...
# conditional entropy
def conditional_entropy(output):
    conditional_ent = -torch.sum(output * torch.log(output + 1e-5), dim=1)
    return torch.mean(conditional_ent)

# marginal entropy
def marginal_entropy(output):
    marginal_ent = -torch.sum(output * torch.log(output + 1e-5))
    return marginal_ent
...
def im_loss(
    ...
    p_y_hat = torch.nn.functional.softmax(logits/temperature, dim=1)
    ce = conditional_entropy(p_y_hat)
    prediction_mean = p_y_hat.mean(dim=0)
    me = marginal_entropy(prediction_mean)
    loss = ce - me
    return loss
